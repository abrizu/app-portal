import './style.css'
import { fetchApplications, fetchApplication, fetchResumes, createApplication, updateApplication, deleteApplication } from './api.js'

// ─── Constants ────────────────────────────────────────────────────────────────

// status options for new application form. 
const STATUS_OPTIONS = [
  'Applying', 'Applied', 'Screening', 'Interviewing', 'Technical'
];

// placeholder for quick status updates on existing applications
const STATUS_OPTIONS_GENERAL = [
  'Applying', 'Applied', 'Screening', 'Interviewing',
...
const SOURCE_OPTIONS = [
  ...
  'Handshake', 'AngelList / Wellfound', 'Referral', 'Career Fair',
];
  ...
  mainContent.innerHTML = `
    <div class="form-page page-enter">
      ...
      <div class="form-page-header">
        ...
        <div>
          <div class="form-page-title">New Application</div>
        ...
        <div class="form-card">
          ...
          <div class="form-grid">
            ...
            <div class="form-group">
              ...
              <input id="job_title" name="job_title" type="text" class="form-input" placeholder="e.g. Software Engineer Intern" required />
            </div>

            <div class="form-group">
              <label class="form-label" for="company_name">Company <span class="required">*</span></label>
              <input id="company_name" name="company_name" type="text" class="form-input" placeholder="e.g. Acme Corp" required />
            </div>

            <div class="form-group">
              <label class="form-label" for="location">Location</label>
              <input id="location" name="location" type="text" class="form-input" placeholder="e.g. New York, NY / Remote" />
            </div>

            <div class="form-group">
              <label class="form-label" for="job_type">Job Type</label>
              <select id="job_type" name="job_type" class="form-select">
                ${JOB_TYPE_OPTIONS.map(o => `<option value="${o}"${o === 'Full-time' ? ' selected' : ''}>${o}</option>`).join('')}
              </select>
            </div>

            <div class="form-group">
              <label class="form-label" for="posting_url">Posting URL</label>
              <input id="posting_url" name="posting_url" type="url" class="form-input" placeholder="https://…" />
            </div>

            <div class="form-group">
              <label class="form-label" for="technologies">Technologies</label>
              <input id="technologies" name="technologies" type="text" class="form-input" placeholder="e.g. Python, React, SQL" />
            </div>
        ...
        <div class="form-card">
          ...
          <div class="form-grid">
            ...
            <div class="form-group">
              ...
              <input id="posting_date" name="posting_date" type="date" class="form-input" />
            </div>

            <div class="form-group">
              <label class="form-label" for="application_date">Application Date</label>
              <input id="application_date" name="application_date" type="date" class="form-input" value="${today}" readonly />
            </div>

            <div class="form-group">
              <label class="form-label" for="status">Status <span class="required">*</span></label>
              <select id="status" name="status" class="form-select">
                ${STATUS_OPTIONS.map(s => `<option value="${s}"${s === 'Applied' ? ' selected' : ''}>${s}</option>`).join('')}
              </select>
            </div>
          ...
          <div class="form-grid">
            ...
            <div class="form-group span-2">
              ...
              <div class="pill-toggle" id="salary-toggle">
                <input type="radio" id="salary-preset" name="salary_mode" value="preset" checked>
                <label for="salary-preset">Preset Range</label>
                <input type="radio" id="salary-custom" name="salary_mode" value="custom">
                <label for="salary-custom">Custom / Other</label>
              </div>
              <div id="salary-preset-wrap">
                <select id="salary_range_select" class="form-select">
                  <option value="">— Select a range —</option>
                  ${SALARY_PRESETS.map(s => `<option value="${s}">${s}</option>`).join('')}
                </select>
              </div>
              <div id="salary-custom-wrap" style="display:none;">
                <input id="salary_range_custom" type="text" class="form-input" placeholder="e.g. $55,000 – $65,000 or Competitive" />
              </div>
            </div>
            ...
            <div class="form-group span-2">
              ...
              <div class="pill-toggle" id="source-toggle">
                <input type="radio" id="source-preset" name="source_mode" value="preset" checked>
                <label for="source-preset">Platform</label>
                <input type="radio" id="source-custom" name="source_mode" value="custom">
                <label for="source-custom">Other</label>
              </div>
              <div id="source-preset-wrap">
                <select id="source_select" class="form-select">
                  <option value="">— Select a source —</option>
                  ${SOURCE_OPTIONS.map(s => `<option value="${s}">${s}</option>`).join('')}
                </select>
              </div>
              <div id="source-custom-wrap" style="display:none;">
                <input id="source_custom" type="text" class="form-input" placeholder="e.g. Professor referral, Discord community…" />
              </div>
            </div>
          ...
          <div class="form-grid">
            ...
            <div class="form-group">
              ...
              <div class="priority-row">
                <input id="priority_score" type="range" min="1" max="10" value="5" step="1"
                  class="priority-slider" style="--pct: 44%;" />
                <span id="priority-display" class="priority-value">5</span>
              </div>
            ...
            <div class="form-group span-2">
              ...
              <textarea id="notes" name="notes" class="form-textarea"
                placeholder="Any additional notes, recruiter contacts, follow-up reminders…"></textarea>
            </div>
...
function getUniqueLocations(apps) {
  const set = new Set();
  apps.forEach(a => { if (a.location) set.add(a.location); });
  return [...set].sort();
}

async function renderApplicationsListView() {
  const mainContent = document.querySelector('.main-content');
  if (!mainContent) return;
    ...
    <tr class="app-row" data-id="${a.id}">
      ...
      <td>
        <div class="row-actions">
          ...
          <button class="btn-icon act-edit" title="Edit" data-id="${a.id}">✏️</button>
